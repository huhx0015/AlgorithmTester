...
object PermutationPalidrome {
    ...
    fun isPalindrome(input: String): Boolean {
        ...
        if (oddCharacters.size <= 1) {
            println("isPalindrome(): $input has permutations that can be palindromes!")
            return true
        } else {
            println("isPalindrome(): $input has no permutations that can be palindromes.")
            return false
        }
    }
